...
@interface PopupSubjectViewController () {
    NSArray *_pickerData;
}

@end

@implementation PopupSubjectViewController
...
- (void) majorDetail {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@api/services/getallmajors/",BASEURL]]];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if(connection == connection1){
        ...
        if([[dict objectForKey:@"status"] isEqualToString:@"1"]){
            ...
            [pickerArray addObject:@"Anything"];
            NSMutableDictionary *firstDict = [[NSMutableDictionary alloc]initWithCapacity:0];
            [firstDict setObject:@"" forKey:@"major_id"];
            [firstDict setObject:@"Anything" forKey:@"major_name"];
            [pickerDataComplete addObject:firstDict];
            for(id obj in [dict objectForKey:@"records"]){
                
                [pickerArray addObject:[obj objectForKey:@"major_name"]];
...
- (IBAction)doneAction:(id)sender {
    NSInteger row = [self.pickerSubject selectedRowInComponent:0];
    
    NSLog(@"%@", [_pickerData objectAtIndex:row]);
    NSLog(@"%@",[pickerDataComplete objectAtIndex:row]);
    
    if([self.delegate respondsToSelector:@selector(selectedPickerValue:)])
    {
         [self.delegate selectedPickerValue:[pickerDataComplete objectAtIndex:row]];
    }
